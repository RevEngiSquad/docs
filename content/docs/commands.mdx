...
<Callout type="warn">
  **Note:** The converted apk file is not signed.
</Callout>
...
<Callout type="warn">
  **Note:** You must register an APK before doing anything. To register an APK, reply to
...
This will give you the information on Signing Block of the APK. For example:
...
<Callout type="warn">
  **Note:** All APK files will be unregistered after 30 minutes.
</Callout>
...
<Callout type="warn">
  **Note:** The specific output formats may vary depending on the input assembly code and
...
<Callout type="warn">
  **Note:** The disassembled output may vary depending on the provided hexadecimal code
...
<Callout type="warn">
  **Note:** Ensure that the input number and encoded strings are correctly formatted for
...
<Callout type="info">
  **Note:** The input file must be an APK, ZIP, or 7Z file containing `libflutter.so` &
  `libapp.so`.
</Callout>
...
<Callout type="info">
  **Note:** You must know the correct decryption key for this command to work. Providing
  an incorrect key will result in failure.
</Callout>

## DEX to C <a id="dex2c"></a>

This command converts an APK file to C code using dex2c.

**Usage:**
To use this command, reply to a ZIP file containing:
1. An APK file
2. A filter.txt file
...
- The ZIP file must contain exactly two files: the APK file and the filter.txt file.
- The filter.txt file should contain the names of the classes you want to decompile, one per line. For structure read [filters](https://github.com/codehasan/dex2c#filters)
- The filter.txt file should not be empty.
</Callout>
...
<Callout type="info">
  **Note:** The success and quality of the conversion depend heavily on the chosen
...
## DEX to Jar <a id="dex2jar"></a>

This command converts DEX files to JAR (Java ARchive) format.
...
<Callout type="info">
  **Note:** This feature is under development and is not yet suitable for real-world or
...
<Callout type="info">
  **Note:** The output will vary depending on the input text or file. For large files,
...
<Callout type="info">
  **Note:** These commands must be used as a reply to the corresponding file.
</Callout>
...
<Callout type="info">
  **Note:** This command must be used as a reply to a `.smali` file. The output will be
...
<Callout type="info">
  **Note:** This command requires an APK file as input and must be used as a reply to that
...
<Callout type="warn">
  **Warning:** Modifying APK files in this way can have unintended consequences and may lead
...
## PairIP Patch <a id="pairip"></a>

This command generates a patch for **PairIP**, which is often used to bypass certain security mechanisms in APKs.

**Usage:**

Reply to an APKS file with the command:

```shell
/pairip
```

After sending this command, you’ll be prompted to choose between two methods:
...
The **Old Method** works exactly as before — no changes have been introduced.  
Refer to prior documentation for its complete workflow.

### New Method

The **New Method** introduces a revised process for applying the PairIP patch.  
Follow the steps below carefully.

#### Step 1: Upload and Process

1. Upload your `.apks` file.  
2. Reply to it with the command:
   ```shell
   /pairip
   ```
3. When prompted, select **New Method**.  
4. The bot will process your APK and generate **two output files**:
   - `<original_name>_merged.apk`
   - `<original_name>_pairip.apk`

You **only need to download the `_pairip.apk`** file. The `_merged.apk` file will be used later in processing and does **not** need to be downloaded.

> **Note:** The generated APK will be **unsigned**.  
> - On **rooted devices**, you can install it if signature verification is disabled.  
> - On **non-root devices**, use virtual environments such as **VMOS**, **VPhoneGaGa**, etc.

#### Step 2: Base APK Replacement

Before launching the app, you’ll need to replace the installed base APK:

- **Using MT Manager**

1. Install the `_pairip.apk` file but **don’t launch it yet**.  
2. Open **MT Manager** and locate the installed app.  
3. View its **APK path**. This is usually something like:
   ```
   /data/app/~~BxxVXv9VabywdisFaGQqVQ==/com.tempmail-JWR28RfeVeG569PXZg69ew==/base.apk
   ```
4. Tap on the path to open the directory.  
5. From your original `.apks` archive (the one you uploaded to the bot), extract the `base.apk`.  
6. Replace the `base.apk` located in the app’s installation directory with this extracted one.

Now, you can **launch your app**.  
If it crashes, proceed to the next step — that’s normal at this stage.

#### Step 3: Retrieve PairIP Data

1. Return to MT Manager and view the app’s **Data directory**.  
   ...
   Example:
   ```
   /data/user/0/com.tempmail
   ```
2. Inside this directory, locate the `dictionary` folder.  
3. Open it and find the file named `pairip.json`.  
4. Upload `pairip.json` to the bot as a **reply** to the `_merged.apk` (the one generated earlier).

The bot will then process your JSON and produce the **PairIP-removed APK** — your final output.

- **Alternative via ADB**

If you prefer command-line tools:

1. Use ADB to find the installed base APK path:
   ```shell
   pm path <package_name>
   ```
2. Follow the same steps as above for replacing the base APK and locating `pairip.json`.

<Callout type="warn">
  **Warning:** Use this command cautiously and only on APKs you have permission to modify. This command is only for educational purposes and should not be used for malicious activities.
</Callout>
...
<Callout type="info">
  **Note:** This command must be used as a reply to an APK file.. The accuracy and detail
  of the results will depend on the capabilities of the chosen scanner and the
  complexity of the APK.
</Callout>

## SSL Patch <a id="sslpatch"></a>

Use this cmd while replying to an `.apk` or `.apks` file to patch SSL pinning in the APK.
...
<Callout type="warn">
  **Note:**
  ...
  - This is not a permanent and full-proof solution; it may not work on all apps. Please don't come crying to us if it doesn't work. We can't help you with that.
...
<Callout type="warn">
  **Note:** The resulting `.apk` file will **not** be signed. You will need to sign it
...
<Callout title="Note">
  **Note:** This command requires an AXML or XML file as input and should be used as a
...
This command lists the third-party libraries used in this project. You can view this information by using the command `/credits`.
